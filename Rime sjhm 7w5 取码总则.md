...
(请安装Unicode大字符集，否则极端罕用字如𧮟㦲𦉭角(善哉四角)等不能显示)  
(如果你的 WORD 软件不能显示极端罕用字，请用记事本(Unicode格式)阅读本文件)  
(调整屏宽度和字体【建议用小四-六号宋体黑体】，使左边的双杠''对齐)  
...
- (7)、 角7
  ...
  - 第二种角7为两画角7，它是单笔相接。它的角尖为7两边延伸部分都可以取码而且视为单笔。例如<font color=red>山</font>4码角7(4个码2277)、但是点3和其他笔形相连时，不构成角7，例如<font color=red>军</font>、<font color=red>冢</font>秃宝盖中取的1码是3不是7。
...
### 【第二.11条】字形形状按照《印刷通用汉字字形》的规定但是注意以下的例子
...
【注意】又旁和又字印刷体汉字笔形有问题   身字和身旁笔形不一样  冒字上边帽字的右上角的笔形有问题，根本不是方块6，鬥字12码不是角7，但多年传统地取冒字上边为方块6，鬥字12码为77了。
...
- (1)由于各种原因，有一些字，虽然根据这里的细则取码是合理的，我们暂视为建议兼容码#，而不放进5码码表。等待时间考验。我们的5码码表2万7千字(包括常用字、次常用字、非常用字和罕用字)还是保留 **徐梦罗** 的原表(和现代汉语词典的新四角基本一致)、
